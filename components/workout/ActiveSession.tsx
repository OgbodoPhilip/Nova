...
import { WorkoutStore } from "@/store/useWorkoutStore";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { CheckCircle2, Plus, Timer, X, Dumbbell } from "lucide-react";
...
export default function ActiveSession() {
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto">
      ...
      <AnimatePresence>
        {exercises.map((ex: { id: Key | null | undefined; name: string | number | bigint | boolean | ReactElement<unknown, string | JSXElementConstructor<any>> | Iterable<ReactNode> | ReactPortal | Promise<string | number | bigint | boolean | ReactPortal | ReactElement<unknown, string | JSXElementConstructor<any>> | Iterable<ReactNode> | null | undefined> | null | undefined; sets: any[]; }) => (
          <motion.div
            key={ex.id}
            ...
            <Card className="border-2 overflow-hidden">
              ...
              <CardContent className="p-0">
                ...
                {ex.sets.map((set, idx) => (
                  <div 
                    key={set.id} 
                    className="grid grid-cols-4 px-6 py-3 items-center hover:bg-muted/30 transition-colors"
                  >
                    <span className="text-sm font-bold">{idx + 1}</span>
                    
                    <Input 
                      type="number" 
                      className="h-8 w-16 bg-transparent border-none focus-visible:ring-1" 
                      placeholder="0"
                    />
      ...
      <Button 
        variant="outline" 
        className="w-full py-8 border-2 border-dashed rounded-2xl gap-2" 
        onClick={() => addExercise("Bench Press")}
      >
...
function useWorkoutStore(): { exercises: any; addSet: any; addExercise: any; isActive: any; startWorkout: any; } {
