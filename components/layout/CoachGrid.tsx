...
const TeamSection: React.FC = () => {
  ...
  return (
    <div
      ref={containerRef}
      className="w-full overflow-x-hidden font-['Syne'] bg-gradient-to-t from-[#FAFAF5] to-[#BEC430] pb-10"
    >
      {/* HERO */}
      ...
      <div className="relative w-full md:grid md:grid-cols-3 lg:grid-cols-4 gap-8 px-6 md:px-12">
        {teamMembers.map((member) => (
          <section
            key={member.id}
            className="team-section h-screen md:h-[650px] flex justify-center items-center p-6 md:p-0"
          >
            <div className="card-content relative w-full h-[85%] md:h-full max-w-[450px] rounded-[2rem] overflow-hidden shadow-2xl">
              <img
                src={member.image}
                alt={member.name}
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black via-black/40 to-transparent flex flex-col justify-end p-8">
                ...
                <p className="text-white/60 italic mt-3">{member.quote}</p>

                {/* BUTTON */}
                <Link
                  href={`/team/${member.id}`}
                  className="mt-6 w-fit px-6 py-3 rounded-full bg-white/10 border border-white/30 text-white text-xs font-bold uppercase tracking-widest hover:bg-[#BEC430] hover:text-black transition-all"
                >
                  View Profile →
                </Link>
              </div>
